...
export default function Navbar() {
  ...
  return (
    <nav className="w-full bg-white h-[91px] flex items-center justify-between px-6 md:px-20 sticky top-0 z-50 border-b border-gray-50">
      {/* Logo */}
      ...
      <div className="hidden md:flex items-center gap-10">
        {navLinks.map((link) => (
          <div key={link.label} className="relative group">
            <Link
              href={link.href}
              className="flex items-center gap-1 text-dark text-[15px] font-medium hover:text-primary transition-colors py-4"
            >
              {link.label}
              {link.hasDropdown && (
                <svg
                ...
                <div className="bg-white shadow-[0_10px_30px_rgba(0,0,0,0.1)] rounded-xl min-w-[240px] py-3 border border-gray-100 overflow-hidden">
                  {link.subLinks.map((sub) => (
                    <Link
                      key={sub.label}
                      href={sub.href}
                      className="block px-6 py-3 text-[14px] text-dark hover:bg-gray-50 hover:text-primary transition-all font-medium"
                    >
                      {sub.label}
                    </Link>
                  ))}
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="absolute top-[91px] left-0 right-0 bg-white shadow-2xl p-6 flex flex-col gap-2 md:hidden border-t border-gray-100 z-40"
          >
            {navLinks.map((link) => (
                      ...
                      <div className="flex flex-col pl-4 bg-gray-50 rounded-lg mt-2">
                        {link.subLinks?.map((sub) => (
                          <Link
                            key={sub.label}
                            href={sub.href}
                            className="text-dark text-[14px] font-medium py-3 border-b border-gray-100 last:border-0"
