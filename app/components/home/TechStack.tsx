  ...
  return (
    <section className="w-full bg-[#F1FCFF] py-10 px-10">
      <div className="max-w-7xl mx-auto flex flex-col items-center gap-16">
        ...
        {/* Tab Selection */}
        <div className="w-full max-w-[1000px] flex border border-[#00161D] rounded-2xl overflow-hidden">
          {Object.keys(techData).map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`flex-1 py-4 font-semibold text-[16px] transition-all flex items-center justify-center gap-2 ${activeTab === tab ? "bg-[#00161D] text-white" : "bg-white text-[#00161D]"
                }`}
            >
              {tab}
              {activeTab === tab && (
